...
function DatesPage() {
  ...
  return <><PageHeading eyebrow="CALENDAR MATH, MADE SIMPLE" title="Date calculator" description="Work with dates without losing a day to timezone math."/><div className="calculator-grid"><section className="tool-panel"><span className="eyebrow">BETWEEN TWO DATES</span><h2>How far apart?</h2><div className="form-row"><label>Start date<input type="date" value={start} onChange={e=>setStart(e.target.value)}/></label><label>End date<input type="date" value={end} onChange={e=>setEnd(e.target.value)}/></label></div><div className="date-results"><div><strong>{dateValid?Math.abs(calDays):'—'}</strong><span>calendar days</span></div><div><strong>{dateValid?`${Math.floor(Math.abs(calDays)/7)}w ${Math.abs(calDays)%7}d`:'—'}</strong><span>weeks + days</span></div><div><strong>{dateValid?Math.abs(diff):'—'}</strong><span>weekdays</span></div></div><p className="field-note">Weekdays are Monday–Friday. Federal and company holidays are not excluded.</p></section><section className="tool-panel"><span className="eyebrow">ADD OR SUBTRACT</span><h2>Find a future date</h2><label>Starting date<input type="date" value={start} onChange={e=>setStart(e.target.value)}/></label><div className="form-row"><label>Number<input type="number" value={amount} min="0" max="10000" onChange={e=>setAmount(Math.max(0,+e.target.value))}/></label><label>Unit<select value={unit} onChange={e=>setUnit(e.target.value as typeof unit)}><option value="days">Calendar days</option><option value="weeks">Weeks</option><option value="business">Business days</option></select></label></div><div className="direction-buttons"><button className={direction===1?'active':''} onClick={()=>setDirection(1)}>Add</button><button className={direction===-1?'active':''} onClick={()=>setDirection(-1)}>Subtract</button></div><div className="result-date">{result?new Intl.DateTimeFormat(undefined,{dateStyle:'long'}).format(new Date(`${result}T12:00:00`)):'Choose a starting date'}</div><p className="field-note">Business days exclude weekends. Holidays are not excluded.</p></section></div></>
}
function PercentPage() {
  ...
  const cards=[{title:'Percent of total',description:'What is A% of B?',result:valid(a,b)?percentOf(number(a),number(b)):null,formula:'A ÷ 100 × B'},{title:'Percent change',description:'Change from A to B',result:valid(a,b)?percentChange(number(a),number(b)):null,formula:'(B − A) ÷ |A| × 100'},{title:'Variance percent',description:'Actual vs. planned',result:valid(a,b)?variancePercent(number(a),number(b)):null,formula:'(Actual − Plan) ÷ |Plan| × 100'}]
  return <><PageHeading eyebrow="NUMBERS, WITH CONTEXT" title="% Calculator" description="Simple formulas for everyday business questions."/><div className="calculator-grid percent-grid">{cards.map(card=><section className="tool-panel" key={card.title}><span className="eyebrow">{card.title.toUpperCase()}</span><h2>{card.description}</h2><div className="form-row"><label>A<input type="number" value={a} onChange={e=>setA(e.target.value)}/></label><label>B<input type="number" value={b} onChange={e=>setB(e.target.value)}/></label></div><div className="calc-result">{card.result===null?'—':`${card.result.toLocaleString(undefined,{maximumFractionDigits:2})}%`}</div><p className="field-note">{card.formula} <button className="text-button" onClick={()=>{setA('');setB('')}}>Reset</button></p></section>)}{(['Margin','Markup'] as const).map(kind=>{const p=number(price),c=number(cost);const answer=valid(price,cost)?(kind==='Margin'?marginPercent(p,c):markupPercent(p,c)):null;return <section className="tool-panel" key={kind}><span className="eyebrow">{kind.toUpperCase()}</span><h2>{kind==='Margin'?'Profit as a share of selling price':'Amount added to cost'}</h2><div className="form-row"><label>Selling price<input type="number" value={price} onChange={e=>setPrice(e.target.value)}/></label><label>Cost<input type="number" value={cost} onChange={e=>setCost(e.target.value)}/></label></div><div className="calc-result">{answer===null?'—':`${answer.toLocaleString(undefined,{maximumFractionDigits:2})}%`}</div><p className="field-note">{kind==='Margin'?'(Price − Cost) ÷ Price':'(Price − Cost) ÷ Cost'} · <button className="text-button" onClick={()=>{setPrice('');setCost('')}}>Reset</button></p></section>})}</div><p className="field-note">A variance uses the absolute plan value as its denominator. Negative and zero values are handled without silently dividing by zero.</p></>
}
