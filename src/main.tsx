...
import { addBusinessDays, agendaIndexAtElapsed, businessDaysBetween, COLOR_PRESETS, COLOR_TOKEN_LABELS, contrastRatio, countdownParts, decodeShare, DEFAULT_CUSTOM_PALETTES, encodeShare, marginPercent, markupPercent, percentChange, percentOf, readData, saveData, STORAGE_KEY, textTransform, variancePercent, type ColorTokens, type Countdown, type DeskData, type EventType, type PaletteMode, type PaletteName, type Theme } from './model'
import './style.css'
import './mobile.css'
...
export function App() {
  ...
  useEffect(() => {
    ...
    const apply = () => {
      const mode: PaletteMode = data.appearance === 'system' ? (media?.matches ? 'dark' : 'light') : data.appearance
      const root = document.documentElement
      const colors = data.palette === 'custom' ? data.customPalettes[mode] : COLOR_PRESETS[data.palette][mode]
      const cssVars: Record<keyof ColorTokens,string> = { page:'--bg',card:'--surface',raised:'--surface2',text:'--text',muted:'--muted',border:'--line',accent:'--accent',onAccent:'--on-accent',accentText:'--accent-dark',accentSoft:'--accent-soft',positive:'--success',warning:'--warning',danger:'--danger',warm:'--warm' }
      root.dataset.theme = mode
      for (const [key, value] of Object.entries(colors) as [keyof ColorTokens,string][]) root.style.setProperty(cssVars[key], value)
    }
    apply()
    media?.addEventListener?.('change', apply)
    return () => media?.removeEventListener?.('change', apply)
  }, [data.appearance,data.palette,data.customPalettes])
...
function SettingsPage({ data, update }: { data: DeskData; update:(p:Partial<DeskData>)=>void }) {
  const [editingMode,setEditingMode] = useState<PaletteMode>(data.appearance==='dark'?'dark':'light')
  const presetNames: Record<Exclude<PaletteName,'custom'>,string> = { sage:'Quiet Sage',ocean:'Coastal Blue',plum:'Orchid',sunset:'Terracotta',rose:'Rosewater',graphite:'Graphite' }
  const paletteChoices = (Object.keys(presetNames) as Exclude<PaletteName,'custom'>[]).map(name=>({id:name as PaletteName,name:presetNames[name],colors:COLOR_PRESETS[name][editingMode]}))
  paletteChoices.push({id:'custom',name:'My custom colors',colors:data.customPalettes[editingMode]})
  const updateColor = (key:keyof ColorTokens,value:string) => { const source=data.palette==='custom'?data.customPalettes:COLOR_PRESETS[data.palette]; update({palette:'custom',customPalettes:{...source,[editingMode]:{...source[editingMode],[key]:value}}}) }
  const activeColors = data.palette==='custom' ? data.customPalettes[editingMode] : COLOR_PRESETS[data.palette][editingMode]
  const contrastChecks = [ ['Main text on page',activeColors.text,activeColors.page],['Main text on cards',activeColors.text,activeColors.card],['Button text on accent',activeColors.onAccent,activeColors.accent] ] as const
  return <><PageHeading eyebrow="MAKE IT YOURS" title="Settings" description="Choose a palette, then fine-tune every important app color."/><div className="settings-list"><section className="settings-item"><div><h2 id="appearance-heading">Appearance</h2><p>Choose light, dark, or follow your device. Your color palette adapts to both modes.</p></div><select aria-labelledby="appearance-heading" value={data.appearance} onChange={e=>update({appearance:e.target.value as DeskData['appearance']})}><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select></section>
    <section className="palette-studio" aria-labelledby="palette-heading"><div className="palette-studio-heading"><div><span className="eyebrow">COLOR STUDIO</span><h2 id="palette-heading">Make Office Desk yours</h2><p>Pick a ready-made palette or customize 14 colors. Changes save on this device and preview as you make them.</p></div></div>
      <div className="palette-choices" role="group" aria-label="Color palettes">{paletteChoices.map(choice=><button key={choice.id} className={`palette-choice ${data.palette===choice.id?'selected':''}`} aria-pressed={data.palette===choice.id} onClick={()=>update({palette:choice.id})}><span className="palette-preview" style={{background:choice.colors.page,borderColor:choice.colors.border}}><i style={{background:choice.colors.accent}}/><i style={{background:choice.colors.accentSoft}}/><i style={{background:choice.colors.text}}/></span><span className="palette-choice-copy"><strong>{choice.name}</strong><small>{choice.id==='custom'?'Build a palette from your own colors':`A ${choice.id==='sage'?'calm':choice.id==='graphite'?'neutral':'colorful'} ${editingMode} palette`}</small></span><span className="palette-dots" aria-hidden="true"><i style={{background:choice.colors.accent}}/><i style={{background:choice.colors.accentText}}/><i style={{background:choice.colors.warm}}/></span></button>)}</div>
      <div className="palette-editor"><div className="palette-editor-heading"><div><h3>Fine-tune colors</h3><p>Edit the light and dark palettes separately. Your choice previews instantly.</p></div><div className="palette-mode-tabs" role="group" aria-label="Choose palette to customize">{(['light','dark'] as const).map(mode=><button key={mode} className={editingMode===mode?'active':''} aria-pressed={editingMode===mode} onClick={()=>setEditingMode(mode)}>{mode==='light'?'Light colors':'Dark colors'}</button>)}</div></div><div className="color-grid">{(Object.keys(COLOR_TOKEN_LABELS) as (keyof ColorTokens)[]).map(key=><label className="color-field" key={key}><span>{COLOR_TOKEN_LABELS[key]}</span><span className="color-picker-wrap"><input aria-label={`${COLOR_TOKEN_LABELS[key]} color`} type="color" value={activeColors[key]} onChange={e=>updateColor(key,e.target.value)}/><code>{activeColors[key].toUpperCase()}</code></span></label>)}</div><div className="contrast-checks"><strong>Text contrast check</strong>{contrastChecks.map(([label,fg,bg])=>{const ratio=contrastRatio(fg,bg);const passes=ratio>=4.5;return <span key={label} className={passes?'contrast-pass':'contrast-warning'}>{passes?'✓':'!'} {label}: {ratio.toFixed(1)}:1 {passes?'':'(choose stronger contrast)'}</span>})}</div><button className="text-button palette-reset" onClick={()=>update({palette:'custom',customPalettes:{...data.customPalettes,[editingMode]:{...DEFAULT_CUSTOM_PALETTES[editingMode]}}})}>Reset {editingMode} colors to Office Desk defaults</button></div>
    </section>
    <section className="settings-item"><div><h2 id="default-sound-heading">Default sound</h2><p>Sound is always opt-in for countdowns and timers.</p></div><label className="switch"><input aria-labelledby="default-sound-heading" type="checkbox" checked={data.sound} onChange={e=>update({sound:e.target.checked})}/><span/></label></section><section className="settings-item"><div><h2>Privacy</h2><p>Your content stays on this device except countdown details you choose to share in a link.</p></div><span className="muted">No accounts or analytics</span></section><section className="settings-item"><div><h2>Install Office Desk</h2><p>Use your browser’s install option to add the app to your device.</p></div><span className="muted">Available in supported browsers</span></section><section className="settings-item"><div><h2>Reset app data</h2><p>Remove saved countdowns, notes, checklist, and preferences from this device.</p></div><button className="danger-button" onClick={()=>{if(confirm('Permanently clear all Office Desk data from this device?')){localStorage.removeItem(STORAGE_KEY);location.reload()}}}>Reset data</button></section><div className="version-note">Office Desk · Version 1.1.0</div></div></>
}
